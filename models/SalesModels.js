...
const createSale = async(product) => {
  connect().then(db => db.collection('sales').insertOne({itensSold: [...product]}))
...
const updateSalesById = async (id, sale) => {
  connect().then(db => db.collection('sales').findOneAndUpdate({_id: ObjectId(id)},
    {$set: { itensSold: sale }},
...
const deleteSalesById = async (id) => {
  connect().then(db => db.collection('sales').findOneAndDelete({_id: ObjectId(id)}))
