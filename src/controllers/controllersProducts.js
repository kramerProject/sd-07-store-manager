...
const createNew = async (req, res) => {
  ...
  try {
    ...
    res.status(magicNumbers[4]).json({ message: 'Internal Server Error' });
  }
};
...
const getAll = async (req, res) => {
  try {
    ...
    res.status(magicNumbers[0]).json(allProducts);
  } catch (err) {
    res.status(magicNumbers[4]).json({ message: 'Internal server error' });
  }
};
...
const getById = async (req, res) => {
  ...
  try {
    ...
    res.status(magicNumbers[0]).json(prodById);
  } catch (err) {
    if (err.code === 'invalid_data') {
      return res.status(magicNumbers[3]).json({ err });
    }
    res.status(magicNumbers[4]).json({ err });
    // res.status(magicNumbers[3]).json({ err });
  }
};
...
const updateById = async (req, res) => {
  ...
  try {
    ...
    res.status(magicNumbers[0]).json(updatedProd);
  } catch (err) {
    if (err.code === 'invalid_data') {
      return res.status(magicNumbers[3]).json({ err });
    }
...
const excludeById = async (req, res) => {
  ...
  try {
    ...
    return res.status(magicNumbers[0]).json(excludedProd);
  } catch (err) {
    if (err.code === 'invalid_data') {
      return res.status(magicNumbers[3]).json({ err });
    }
...
// 		res.status(magicNumbers[0]).json(result);
// 	} catch (err) {
// 		console.log(err);
// 		res.status(magicNumbers[4]).json({ message: err.message });
// 	}
// };
